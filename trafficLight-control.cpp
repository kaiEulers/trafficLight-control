...
private:
	...
	{
		FILE *fpipe;
		char line[256] = {}; // size of Line should be smaller than command
		...
		//Check line[] returned is a number
		for (char* p = line; *p != '\n'; p++)
		{
			if (
				*p != '1' && *p != '2' && *p != '3' && *p != '4' && *p != '5' &&
...
public:
	...
	short get_portB()
	{
		portB = read_write("ousb -r io PORTB");

		return portB;
	}
	short set_portB(unsigned short x)
	{
		stringstream cmdStream;
		cmdStream << "ousb -r io PORTB " << x;
		string cmdString = cmdStream.str();

		portB = read_write(cmdString.c_str());
...
int main(int argc, char *argv[])
{
	try
	{
		...
		else if (argc == 3)
		{
			...
			if (argv[1][0] == 'B')
			{
				//Range Check argv2 for B
				unsigned short num = (unsigned short)atoi(argv[2]);
				rngChk_arg2B(num);

				//Set ousb board value
				OUSB board;
				cout << board.set_portB(num) << endl;
			}
			else
			{
				//Range Check argv2 for RYG
				unsigned short numTrans = (unsigned short)atoi(argv[2]);
				rngChk_arg2RYG(numTrans);

				//Initialise TrafficLight
				TrafficLight light;
				if (argv[1][0] == 'R')
				{
					light.red();
